<template>
  <p class="text-gray-300 text-xs mb-2 text-center whitespace-nowrap">
  ...
  <p class="text-gray-200 text-xs mb-1">
    <span class="mr-1">{{text.total}}
      <span class="text-blue-400">{{detail.total}}</span> {{text.times}}
    </span>
    <span v-if="type !== '100'">{{text.sum}}<span class="mx-1 text-green-400">{{detail.countMio}}</span>{{text.no5star}}</span>
  </p>
  <p class="text-gray-200 text-xs mb-1">
  ...
  <p class="text-gray-200 text-xs mb-1" v-if="detail.ssrPos.length">
    {{text.history}}{{colon}}
  ...
  <p v-if="detail.ssrPos.length" class="text-gray-200 text-xs">{{text.average}}{{colon}}<span class="text-green-400">{{avg5(detail.ssrPos)}}</span></p>
</template>
